...
use Hotel\Application\Interfaces\IDashboardQueryService; // <--- PHẢI IMPLEMENT INTERFACE NÀY
use Hotel\Core\Database\IDatabaseConnection;
...
class MySqlDashboardQueryService implements IDashboardQueryService // <--- ĐÃ SỬA
{
    private $db;

    // ===>>> LỖI CỦA BẠN Ở ĐÂY (ĐÃ SỬA) <<<===
    // Đã thay thế '~' bằng 'IDatabaseConnection'
    public function __construct(IDatabaseConnection $db)
    {
        $this->db = $db->getConnection(); // Lấy đối tượng PDO
    }
    // ===>>> KẾT THÚC SỬA <<<===
